<body class="bg-gray-50">
    ...
    <div class="p-4 sm:ml-60">
        <div class="mt-16">
            ...
            <div class="hidden lg:block bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200" id="employeeTableBody"></tbody>
                    </table>
                </div>
            ...
            <!-- Mobile Card View -->
            <div class="lg:hidden space-y-3" id="mobileCardsContainer"></div>
        </div>
    </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            function canDeleteUser(targetEmployeeType) {
                ...
                if (currentType === 'admin') return targetType === 'staff' || targetType === 'cashier';
                if (currentType === 'owner') return targetType !== 'owner';
                return false;
            }

            function deleteUser(userId, employeeName, btn) {
                if (typeof ButtonLoader !== 'undefined') ButtonLoader.start(btn, '');
                ...
                $.ajax({
                    ...
                    data: JSON.stringify({ user_id: userId, privilege_level: currentUser.employeeType }),
                    success: function (response) {
                        if (typeof ButtonLoader !== 'undefined') ButtonLoader.stop(btn);
                        if (response.success) {
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right">
                            <div class="inline-flex gap-2">
                                <button type="button" class="btn-restore-user text-green-700 bg-green-50 border border-green-200 rounded px-3 py-1.5 text-xs font-medium hover:bg-green-100"
                                    data-user-id="${user.user_id}" data-name="${fullName}">Restore</button>
                                <button type="button" class="btn-hard-delete-user text-red-700 bg-red-50 border border-red-200 rounded px-3 py-1.5 text-xs font-medium hover:bg-red-100"
                                    data-user-id="${user.user_id}" data-name="${fullName}">Delete Permanently</button>
                            </div>
                        </td>
                    </tr>
                `;
            ...
            function changeUserRole(userId, newRole, btn) {
                if (typeof ButtonLoader !== 'undefined') ButtonLoader.start(btn, 'Saving...');
            ...
            function updateStatsCards(users) {
                $('#statTotalEmployees').text(users.length);
                $('#statActiveEmployees').text(users.length);
                $('#statAdminCount').text(users.filter(u => (u.employee_type || '').toLowerCase() === 'admin').length);
            }
            ...
            function renderEmployees(users) {
                ...
                paginatedUsers.forEach(function (user) {
                    ...
                    <tr class="hover:bg-gray-50" data-user-id="${user.user_id}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-700">${role}</span>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-center gap-2">
                                <button type="button" class="text-gray-600 py-2 px-3 bg-gray-100 rounded border border-gray-300 hover:text-gray-800 btn-view" title="View"
                                    data-user-id="${user.user_id}" data-firstname="${user.firstname || ''}"
                                    data-middlename="${user.middlename || ''}" data-lastname="${user.lastname || ''}"
                                    data-email="${user.email || ''}" data-username="${user.username || ''}"
                                    data-phone="${user.phone_number || ''}" data-role="${user.employee_type || ''}"
                                    data-gender="${user.gender || ''}" data-birthdate="${user.birthdate || ''}"
                                ...
                                <button type="button" class="text-red-600 py-2 px-3 bg-gray-100 rounded border border-gray-300 hover:text-red-800 btn-delete"
                                    data-user-id="${user.user_id}" data-name="${fullName}">
            ...
            function renderPagination(totalItems, totalPages, startIndex, endIndex) {
                ...
                <div class="text-sm text-gray-500">
                    ...
                    <span class="font-medium">${totalItems}</span> employees
                </div>
                <div class="flex gap-1" id="paginationButtons">
                    <button class="px-3 py-1 text-sm border border-gray-200 rounded hover:bg-gray-50 disabled:opacity-50 btn-page-prev" ${currentPage === 1 ? 'disabled' : ''}>&laquo; Prev</button>
            `;
                ...
                paginationHTML += `
                    <button class="px-3 py-1 text-sm border border-gray-200 rounded hover:bg-gray-50 disabled:opacity-50 btn-page-next" ${currentPage === totalPages ? 'disabled' : ''}>Next &raquo;</button>
                </div>
            `;

                $('#paginationSection').html(paginationHTML);

                $('.btn-page').off('click').on('click', function () { currentPage = parseInt($(this).data('page')); applyFilters(); });
                $('.btn-page-prev').off('click').on('click', function () { if (currentPage > 1) { currentPage--; applyFilters(); } });
                $('.btn-page-next').off('click').on('click', function () { if (currentPage < totalPages) { currentPage++; applyFilters(); } });
            }
            ...
            function attachEventHandlers() {
                $('.btn-view').off('click').on('click', function () {
                    ...
                    $('#viewInitials').text(getInitials(firstname, lastname));
                ...
                $('.btn-edit-role').off('click').on('click', function () {
                    ...
                    $('#roleModalInitials').text(getInitials(firstname, lastname));
                    $('#roleModalName').text(`${firstname} ${lastname}`.trim());
                    $('#roleModalEmail').text(btn.data('email') || 'N/A');
                    ...
                    if (currentEmployeeType === 'owner') {
                        roleOptions += '<option value="owner">Owner</option><option value="admin">Admin</option><option value="staff">Staff</option>';
                    } else if (currentEmployeeType === 'admin') {
                        roleOptions += '<option value="admin">Admin</option><option value="staff">Staff</option>';
                    }
                    $('#newRole').html(roleOptions);
                    if (userRole) $('#newRole').val(userRole.toLowerCase());

                    $('#changeRoleModal').removeClass('hidden').addClass('flex');
                    syncManageEmployeeBodyScrollLock();
                    setTimeout(() => { $('#newRole').focus(); }, 100);
                });
                ...
                $('.btn-delete').off('click').on('click', function () {
                    ...
                    if (typeof ButtonLoader !== 'undefined' && ButtonLoader.isLoading(btn)) return;
                    Confirm.delete(
            ...
            // Search
            $('#searchInput').on('keyup', function () { applyFilters(); });

            // Filter
            $('#roleFilter').on('change', function () { currentPage = 1; applyFilters(); });
            ...
            function applyFilters() {
                ...
                if (roleFilter !== 'all') {
                    filteredUsers = filteredUsers.filter(user => (user.employee_type || '').toLowerCase() === roleFilter);
                }
            ...
            $('#confirmRoleChange').on('click', function () {
                ...
                if (typeof ButtonLoader !== 'undefined' && ButtonLoader.isLoading(btn)) return;
                if (!newRole) { Toast.warning('Please select a role'); return; }
                if (newRole.toLowerCase() === currentRole.toLowerCase()) { Toast.info('Employee already has this role'); return; }
