        ...
        <ul class="space-y-2 font-medium flex-1">
            ...
            <li class="pt-2 mt-2 border-t border-gray-100">
                <a href="<?= base_url('RawMaterials') ?>" class="flex items-center p-2 text-gray-900 rounded-lg hover:bg-secondary group transition-colors duration-300 <?= ($current === 'rawmaterials') ? 'bg-primary' : '' ?>">
                    ...
                    <span class="flex-1 ms-3 whitespace-nowrap group-hover:text-gray-200 transition-colors duration-300 <?= ($current === 'rawmaterials') ? 'text-white' : '' ?>">Material Costing</span>
                </a>
            </li>
            <li class="pt-2 mt-2 border-t border-gray-100">
                <a href="<?= base_url('StockInitial') ?>" class="flex items-center p-2 text-gray-900 rounded-lg hover:bg-secondary group transition-colors duration-300 <?= ($current === 'stockinitial') ? 'bg-primary' : '' ?>">
                    <svg class="w-6 h-6 <?= ($current === 'stockinitial') ? 'text-white' : 'text-gray-900' ?> transition duration-300 group-hover:text-gray-200" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
                    </svg>
                    <span class="flex-1 ms-3 whitespace-nowrap group-hover:text-gray-200 transition-colors duration-300 <?= ($current === 'stockinitial') ? 'text-white' : '' ?>">Stock Initial</span>
                </a>
            </li>
            <?php if (session('employee_type') === 'admin' || session('employee_type') === 'owner'): ?>
                <li class="pt-2 mt-2 border-t border-gray-100">
