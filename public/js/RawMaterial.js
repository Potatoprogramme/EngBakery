...
$(document).ready(function () {
  ...
  function updateCostCalculations() {
    ...
    // Set base unit label
    $("#cost_unit_label").text(
      unit === "pcs" ? "per pc" : "per " + (unit === "grams" ? "gram" : unit),
    );
  ...
  function loadMaterials() {
    $.ajax({
      ...
      success: function (response) {
        if (response.success && response.data && response.data.length > 0) {
          ...
          $("#materialsTableBody").html(
            '<tr><td colspan="8" class="px-6 py-8 text-center text-gray-500"><i class="fas fa-box-open text-4xl mb-3 block"></i><p>No material data available</p></td></tr>',
          );
          $("#materialsCardsContainer").html(
            '<div class="p-8 bg-white rounded-lg shadow-md text-center text-gray-500"><i class="fas fa-box-open text-4xl mb-3"></i><p>No material data available</p></div>',
          );
      ...
      error: function (xhr, status, error) {
        ...
        $("#materialsTableBody").html("");
        $("#materialsCardsContainer").html(
          '<div class="p-8 bg-white rounded-lg shadow-md text-center text-gray-500"><i class="fas fa-exclamation-triangle text-4xl mb-3"></i><p>Error loading materials</p></div>',
        );
        $("#mobilePagination").html("");
      },
    });
  ...
  function renderDesktopTable(materials) {
    ...
    // Remove any lingering simple-datatables wrapper to ensure a clean table
    var wrapper = document.querySelector(".datatable-wrapper");
    if (wrapper) {
    ...
    materials.forEach(function (mat) {
      ...
      rows +=
        '<tr class="hover:bg-gray-50 cursor-pointer border-b" data-category="' +
        (mat.category_id || "") +
        '">';
      rows +=
      ...
      rows += '<td class="px-6 py-4 ">';
      rows +=
        '<button class="text-gray-600 py-2 px-3 bg-gray-100 rounded border border-gray-300 hover:text-gray-800 me-2 btn-view" data-id="' +
        mat.material_id +
        '" title="View"><i class="fas fa-eye"></i></button>';
      rows +=
        '<button class="text-blue-600 py-2 px-3 bg-gray-100 rounded border border-gray-300 hover:text-blue-800 me-2 btn-edit" data-id="' +
        mat.material_id +
    ...
    var tableElement = document.getElementById("selection-table");
    if (tableElement && typeof simpleDatatables !== "undefined") {
      try {
        ...
        var container = document.querySelector(".datatable-container");
        if (container) {
        ...
        var thead = document.querySelector(".datatable-table thead");
        if (thead) {
        ...
        document
          .querySelectorAll(".datatable-table thead th:first-child")
          .forEach(function (th) {
            th.classList.add("sticky", "left-0", "bg-white", "z-20");
          });
        document
          .querySelectorAll(".datatable-table tbody td:first-child")
          .forEach(function (td) {
        ...
        console.error("DataTable init error:", e.message);
        dataTable = null;
      }
    }
  }
  ...
  $(document).on(
    "click",
    "#mobilePagination button:not([disabled])",
    function () {
  ...
  $("#mobileSearch").on("input", function () {
    ...
    if (searchTerm === "") {
      ...
      filteredMaterials = allMaterials.filter(function (mat) {
        return (
          mat.material_name.toLowerCase().includes(searchTerm) ||
          (mat.category_name &&
            mat.category_name.toLowerCase().includes(searchTerm)) ||
  ...
  function applyDatatableStyles() {
    ...
    if (datatableDropdown) {
      datatableDropdown.classList.add(
        "w-full",
        "sm:w-auto",
        "text-center",
        "sm:text-left",
  ...
  $("#addMaterialForm").on("submit", function (e) {
    ...
    if (
      typeof ButtonLoader !== "undefined" &&
      ButtonLoader.isLoading(submitBtn)
    ) {
      return;
    }
    ...
    if (materialNameExists) {
      Toast.warning(
        "Material name already exists. Please use a different name.",
      );
      return;
    }
      ...
      success: function (response) {
        if (response.success) {
          ...
          const unitLabel =
            unit === "pcs"
              ? "per pc"
              : "per " + (unit === "grams" ? "gram" : unit);
          $("#view_cost_per_unit").text(
            "\u20B1 " + costPU.toFixed(3) + " " + unitLabel,
          );
          ...
          if (conv && initQty > 0) {
            ...
            $("#view_converted_cost").text(
              "\u20B1 " +
                parseFloat(costPerLarge).toLocaleString("en-US", {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                }),
            );
  ...
  $("#btnViewEditMaterial").on("click", function () {
    if (currentViewMaterialId) {
      ...
      $('.btn-edit[data-id="' + currentViewMaterialId + '"]')
        .first()
        .trigger("click");
      // Fallback: if button not found in DOM (paginated), load directly
      if (
        $('.btn-edit[data-id="' + currentViewMaterialId + '"]').length === 0
      ) {
        ...
        $.ajax({
          ...
          success: function (response) {
            if (response.success) {
              ...
              $("#cost_per_unit").val(
                parseFloat(mat.cost_per_unit || 0).toFixed(3),
              );
  ...
  $("#apply-filters").on("click", function () {
    ...
    if (filteredMaterials.length > 0) {
      ...
      $("#materialsTableBody").html(
        '<tr><td colspan="8" class="px-6 py-8 text-center text-gray-500"><i class="fas fa-filter text-4xl mb-3 block"></i><p>No materials found for the selected category</p></td></tr>',
      );
    }
