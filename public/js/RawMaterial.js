...
$(document).ready(function() {
    ...
    function closeModal() {
        ...
        $('#material_quantity').prop('readonly', false).removeClass('bg-gray-100 cursor-not-allowed');
    ...
    function renderDesktopTable(materials) {
        ...
        materials.forEach(function(mat) {
            ...
            const used = Math.max(0, initialQty - qty);
            const pct = initialQty > 0 ? (qty / initialQty) * 100 : 100;

            let barColor = 'bg-emerald-400';
            let barTrack = 'bg-emerald-100';
            let remainText = 'text-gray-700';
            const maxRef = initialQty > 0 ? initialQty : Math.max(qty, 1);
            let barWidth = Math.min(100, (qty / maxRef) * 100);
            ...
            rows += hasStock ? initialQty.toLocaleString('en-US', {maximumFractionDigits: 2}) : '<span class="text-xs bg-gray-200 text-gray-500 px-2 py-0.5 rounded">No stock</span>';
            rows += '</td>';

            // Used column
            rows += '<td class="px-6 py-4 tabular-nums text-sm">';
            rows += hasStock ? '<span class="text-orange-600">' + used.toLocaleString('en-US', {maximumFractionDigits: 2}) + '</span>' : '-';
            rows += '</td>';
            ...
            if (!hasStock) {
                ...
                rows += '<span class="' + remainText + ' tabular-nums text-sm min-w-[2.5rem]">' + qty.toLocaleString('en-US', {maximumFractionDigits: 2}) + '</span>';
            ...
            rows += '<td class="px-6 py-4 ">';
            rows += '<button class="text-emerald-600 py-2 px-3 bg-gray-100 rounded border border-gray-300 hover:text-emerald-800 me-2 btn-restock" data-id="' + mat.material_id + '" data-name="' + mat.material_name + '" data-unit="' + mat.unit + '" data-initial="' + initialQty + '" data-remaining="' + qty + '" title="Restock"><i class="fas fa-plus-circle"></i></button>';
                ...
                <div class="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200">
                    <div class="p-4">
                        ...
                        ${(() => {
                            ...
                            const initQ = parseFloat(mat.initial_qty) || 0;
                            const hasStock = parseInt(mat.has_stock) === 1;
                            const usedQ = Math.max(0, initQ - q);
                            const maxRef = initQ > 0 ? initQ : Math.max(q, 1);
                            const pct = initQ > 0 ? (q / initQ) * 100 : 100;
                            let barColor = 'bg-emerald-400', barTrack = 'bg-emerald-100';
                            let barW = Math.min(100, (q / maxRef) * 100);
                            ...
                            const initDisplay = hasStock ? initQ.toLocaleString('en-US', {maximumFractionDigits: 2}) + ' ' + mat.unit : '<span class="text-xs text-gray-400">No stock</span>';
                            const usedDisplay = hasStock ? usedQ.toLocaleString('en-US', {maximumFractionDigits: 2}) + ' ' + mat.unit : '-';
                            const remainDisplay = hasStock ? q.toLocaleString('en-US', {maximumFractionDigits: 2}) + ' ' + mat.unit : '-';
                            return `
                            ...
                            <div class="grid grid-cols-1 gap-2 mb-3">
                                <div class="bg-primary/10 rounded-lg p-2 flex justify-between items-center">
                        ...
                        <div class="flex gap-2 pt-2 border-t border-gray-100">
                            <button class="flex-1 flex items-center justify-center gap-2 py-2 px-3 text-sm font-medium text-emerald-600 bg-emerald-50 rounded-lg hover:bg-emerald-100 btn-restock" data-id="${mat.material_id}" data-name="${mat.material_name}" data-unit="${mat.unit}" data-initial="${parseFloat(mat.initial_qty) || 0}" data-remaining="${parseFloat(mat.material_quantity) || 0}">
    ...
    $('#addMaterialForm').on('submit', function(e) {
        ...
        if (!materialId) {
            formData.material_quantity = $('#material_quantity').val();
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $('#unit').val(mat.unit);
                    // Quantity is read-only in edit mode (managed by deductions & restock)
                    $('#material_quantity').val(parseFloat(mat.material_quantity).toLocaleString('en-US', {maximumFractionDigits: 2}));
                    $('#material_quantity').prop('readonly', true).addClass('bg-gray-100 cursor-not-allowed');
                    $('#qty_readonly_hint').removeClass('hidden');
